...
class Effects(Entity):
    ...
    def __init__(self, position=Vec3(0,0,0), file='effects/dust.ptf', life=1, deathtime=5, parent=None, force=None):
...
if __name__ == "__main__":    
    ...
    def create_explosion(position):
        ...
        fire = Effects(position=position, file=r'effects\fire_sphere.ptf')
        fire.scale_z = 2
        fire.rotation_y = 180
        fire.rotation_x = -11
        fire.y += 0.7
        ...
        smoke = Effects(parent=cannon, position=position, file=r'effects\smoke.ptf')
        smoke.z -= 1
        smoke.scale=3
        return
