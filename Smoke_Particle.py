...
class Particle_Smoke(Entity):
    ...
    @staticmethod
    def move(entity):
         entity.animate_scale(Vec3(0,0,0), duration=.01)
         entity.visible = True
...
#           Particle_Smoke(pos, vel, clr, txr)
...
if __name__ == "__main__":
    ...
    def input(key):
        if key == 'left mouse down':
            e = Entity(model='cube', scale=1,
                       position=player.position + Vec3(0, 1.5, 0) + player.camera_pivot.forward * 10, collider='box')
            Particle_Smoke.move(e)
